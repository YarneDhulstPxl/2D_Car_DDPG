...
class Actor(object):
    ...
    def _build_net(self, s, scope, trainable):
        with tf.compat.v1.variable_scope(scope):
            init_w = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
            init_b = tf.compat.v1.constant_initializer(0.001)
            net = tf.compat.v1.layers.dense(s, 100, activation=tf.nn.relu,
                                  kernel_initializer=init_w, bias_initializer=init_b, name='l1',
                                  trainable=trainable)
            net = tf.compat.v1.layers.dense(net, 20, activation=tf.nn.relu,
